...
export default function StockCategoryPage() {
  ...
  function refresh() {
    api.listStockCategories().then(setRows).catch((e) => setError(e.message))
  }
  useEffect(refresh, [])
  ...
  async function onCreate(e: FormEvent) {
    ...
    try {
      await api.createStockCategory({ code, name })
      setCode(''); setName('')
      refresh()
  ...
  async function toggle(row: StockSetupRow) {
    setError(null)
    try { await api.toggleStockCategory(row.id); refresh() }
    catch (err) { setError(err instanceof Error ? err.message : 'Failed') }
  }
  ...
  async function saveEdit() {
    ...
    try {
      await api.updateStockCategory(editId, { code: editCode, name: editName })
      setEditId(null)
      refresh()
    } catch (err) { setError(err instanceof Error ? err.message : 'Failed') }
  }
      ...
      <div style={{ overflowX: 'auto' }}>
        <table>
          ...
          <tbody>
            ...
            {!rows.length && <tr><td colSpan={4} style={{ textAlign: 'center' }}>No categories yet</td></tr>}
          </tbody>
        </table>
      </div>
    </div>
