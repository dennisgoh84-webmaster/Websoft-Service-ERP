...
export default function StockUsagePage() {
  ...
  function refresh() {
    api.listStockUsages().then(setRows).catch((e) => setError(e.message))
  }
  useEffect(refresh, [])
  ...
  async function onCreate(e: FormEvent) {
    ...
    try {
      await api.createStockUsage({ code, name })
      setCode(''); setName('')
      refresh()
  ...
  async function toggle(row: StockSetupRow) {
    setError(null)
    try { await api.toggleStockUsage(row.id); refresh() }
    catch (err) { setError(err instanceof Error ? err.message : 'Failed') }
  }
  ...
  async function saveEdit() {
    ...
    try {
      await api.updateStockUsage(editId, { code: editCode, name: editName })
      setEditId(null)
      refresh()
    } catch (err) { setError(err instanceof Error ? err.message : 'Failed') }
  }
      ...
      <div style={{ overflowX: 'auto' }}>
        <table>
          ...
          <tbody>
            ...
            {!rows.length && <tr><td colSpan={4} style={{ textAlign: 'center' }}>No usages yet</td></tr>}
          </tbody>
        </table>
      </div>
    </div>
