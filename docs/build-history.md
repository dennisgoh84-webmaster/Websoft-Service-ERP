...
The module-by-module record of how Websoft Service ERP was built, moved
here word for word from CLAUDE.md's old "Status" section on 2026-09-26
so CLAUDE.md can stay a short set of rules. Entries are history: file
paths under `backend/` refer to the retired Python backend and resolve
in git history, not the working tree. Newer entries are appended at the
end; for what is pending see [backlog.md](backlog.md).

## Record up to 2026-09-26
...
- **Times fixed at the root**: the database session now runs in
  `APP_TIMEZONE`; stored times the app had written eight hours ahead
  were corrected (migration `2026_09_30_002500`), and sales invoice
  vouchers dated a day early moved to their Singapore date
  (`2026_09_30_002800`).
- **Module Control tidy-up**: Purchasing, Projects, Hardware
  Management, Inventory and Integrations keys removed; Commission
  Management owns every commission feature; AI Assistant hidden
  everywhere unless switched on; Goods Issue Note got its screen.
- **Prospect / Leads** settled: pipeline stages, accepting a quotation
  wins the prospect, activities voided never deleted.
- **Accounting answers**: PO and credit note approval limits on each
  Company / Individual; a bill for a different amount than its PO is
  paid on the billed amount; write-offs post to 6700 Bad debts written
  off; revenue on invoice (no deferral); stock movements never post to
  the GL.
- **GST F5 workflow** (open item 4b.4): GST Calculation on a locked
  period keeps the Form 5 boxes and their documents; the GST Return and
  GST Supporting Listing read only what is kept.
- **Company / Individual ID and name in FULL CAPITALS**, whatever way
  they arrive; existing records converted (`2026_09_30_002900`).
- **Date boxes**: slashes fill in as digits are typed, the calendar
